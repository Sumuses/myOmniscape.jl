...
function run_omniscape(
        ...
        condition3::MissingArray{T, 2} where T <: Number = MissingArray{Float64, 2}(undef, 1, 1),
        condition4::MissingArray{T, 2} where T <: Number = MissingArray{Float64, 2}(undef, 1, 1),
        condition1_future::MissingArray{T, 2} where T <: Number = condition1,
        condition2_future::MissingArray{T, 2} where T <: Number = condition2,
        condition3_future::MissingArray{T, 2} where T <: Number = condition3,
        condition4_future::MissingArray{T, 2} where T <: Number = condition4,
        wkt::String = "",
    ...
    conditions = Conditions(cfg["comparison1"],
                            ...
                            parse(Float64, cfg["condition3_lower"]),
                            parse(Float64, cfg["condition3_upper"]),
                            parse(Float64, cfg["condition4_lower"]),
                            parse(Float64, cfg["condition4_upper"]))

    # a bit of a hack, but ensures compare_to_future can work in both methods
    # for run_omniscape
    if compare_to_future == "1"                      #为什么是相反的？？？？
        condition2_future = condition2
    elseif compare_to_future == "2"
        condition1_future = condition1
    elseif compare_to_future == "123"
        condition4_future = condition4
    elseif compare_to_future == "none"
        condition1_future = condition1
        condition2_future = condition2
        condition3_future = condition3
        condition4_future = condition4
    ...
    condition_layers = ConditionLayers(condition1, condition1_future,
                                       ...
                                       condition3, condition3_future,
                                       condition4, condition4_future)
    ...
    condition3 = nothing
    condition3_future = nothing
    condition4 = nothing
    condition4_future = nothing
    GC.gc()
...
function run_omniscape(path::String)
    ...
    if os_flags.conditional
        ...
        if compare_to_future == "1" || compare_to_future == "both" || compare_to_future == "123"
        ...
        if n_conditions == 4
            condition2_raster = read_raster("$(cfg["condition2_file"])", precision)
            condition2 = condition2_raster[1]
            condition3_raster = read_raster("$(cfg["condition3_file"])", precision)
            condition3 = condition4_raster[1]
            condition4_raster = read_raster("$(cfg["condition4_file"])", precision)
            condition4 = condition4_raster[1]
            # Check for raster alignment
            check_raster_alignment(resistance_raster, condition2_raster,
                                   "resistance_file", "condition2_file",
                                   allow_different_projections) && return
            check_raster_alignment(resistance_raster, condition3_raster,
                                   "resistance_file", "condition3_file",
                                   allow_different_projections) && return
            check_raster_alignment(resistance_raster, condition4_raster,
                                   "resistance_file", "condition4_file",
                                   allow_different_projections) && return

            # get rid of unneeded raster to save memory
            condition2_raster = nothing
            condition3_raster = nothing
            condition4_raster = nothing

            if compare_to_future == "123" || compare_to_future == "both"
                condition2_future_raster = read_raster("$(cfg["condition2_future_file"])", precision)
                condition2_future = condition2_future_raster[1]
                condition3_future_raster = read_raster("$(cfg["condition3_future_file"])", precision)
                condition3_future = condition3_future_raster[1]
                condition4_future_raster = read_raster("$(cfg["condition4_future_file"])", precision)
                condition4_future = condition4_future_raster[1]

                # Check for raster alignment
                check_raster_alignment(resistance_raster, condition2_future_raster,
                                       "resistance_file", "condition2_future_file",
                                       allow_different_projections) && return
                check_raster_alignment(resistance_raster, condition3_future_raster,
                                       "resistance_file", "condition3_future_file",
                                       allow_different_projections) && return
                check_raster_alignment(resistance_raster, condition4_future_raster,
                                       "resistance_file", "condition4_future_file",
                                       allow_different_projections) && return

                # get rid of unneeded raster to save memory
                condition2_future_raster = nothing
                condition3_future_raster = nothing
                condition4_future_raster = nothing
            else
                condition2_future = MissingArray{precision, 2}(undef, 1, 1)
                condition3_future = MissingArray{precision, 2}(undef, 1, 1)
                condition4_future = MissingArray{precision, 2}(undef, 1, 1)
            end
    else
        condition1 = MissingArray{precision, 2}(undef, 1, 1)
        condition2 = MissingArray{precision, 2}(undef, 1, 1)
        condition3 = MissingArray{precision, 2}(undef, 1, 1)
        condition4 = MissingArray{precision, 2}(undef, 1, 1)
        condition1_future = condition1
        condition2_future = condition2
        condition3_future = condition3
        condition4_future = condition4
    ...
    run_omniscape(
        ...
        condition3_future = condition3_future,
        condition4_future = condition4_future,
